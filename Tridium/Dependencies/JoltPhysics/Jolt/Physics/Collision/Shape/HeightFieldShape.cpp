		...
		do
		{
			...
			if (level >= max_level)
			{
				...
				for (int block = 0; block < 4; ++block)
				{
					...
					if (bounds.IsValid())
						DebugRenderer::sInstance->DrawWireBox(bounds, Color4::sYellow);
				}
					...
					for (uint32 v_y = block_min_y; v_y < block_max_y; ++v_y)
						for (uint32 v_x = block_min_x; v_x < block_max_x; ++v_x)
						{
							...
							if (!start_no_collision[0] && !start_no_collision[block_size_plus_1 + 1])
							{
								...
								#ifdef JPH_DEBUG_HEIGHT_FIELD
									DebugRenderer::sInstance->DrawWireTriangle(RVec3(v0), RVec3(v1), RVec3(v2), Color4::sWhite);
								#endif
			...
			else
			{
				...
				for (int i = 0; i < 4; ++i)
				{
					...
					if (b.IsValid())
						DebugRenderer::sInstance->DrawWireBox(b, Color4::sGreen);
				}
			#endif
