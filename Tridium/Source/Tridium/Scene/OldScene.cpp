...
namespace Tridium {
	...
	OldScene::OldScene(const String& name)
		: m_Name( name ), m_SceneRenderer(*this)
	{
		Initialize();
	}

	OldScene::OldScene( const OldScene& a_Other )
			...
			for ( auto [id, srcStorage] : src.storage() ) 
			{
				...
				for ( auto it = srcStorage.rbegin(); it != srcStorage.rend(); ++it ) 
				{
					...
					if ( Component* dstAsComponent = dstComponent.try_cast<Component>() )
						dstAsComponent->m_EntityID = OldGameObject( entity );
				}
			}
	...
	void OldScene::DestroyGameObject( OldGameObject a_GameObject )
	{
	...
	OldGameObject OldScene::InstantiateGameObject( const String& a_Name )
	{
	...
	void OldScene::CopyGameObject( OldGameObject a_Destination, OldGameObject a_Source )
	{
	...
	bool OldScene::IsGameObjectValid( OldGameObject a_GameObject ) const
	{
	...
	OldGameObject OldScene::FindGameObjectByTag( const String& a_Tag ) const
	{
		...
		for ( auto&& [ entity, tagComponent ] : view.each() )
		{
			if ( tagComponent.Tag == a_Tag )
				return OldGameObject( entity );
		}

		return OldGameObject{};
	}

	std::vector<OldGameObject> OldScene::FindAllGameObjectsByTag( const String& a_Tag ) const
	{
		...
		{
			if ( Tag.Tag == a_Tag )
				gameObjects.push_back( OldGameObject( entity ) );
		}

		return gameObjects;
	}

	void OldScene::InitAllComponentTypes()
	{
	...
	void OldScene::SendSceneEvent( const SceneEventPayload& a_EventPayload )
	{
		PROFILE_FUNCTION( ProfilerCategory::Scene );
		for ( auto&& [type, system] : m_Systems )
		{
	...
	OldGameObject OldScene::GetMainCameraGameObject() const
	{
		return OldGameObject( m_MainCamera );
	}

	void OldScene::SetMainCamera( OldGameObject a_CameraGameObject )
	{
