...
namespace Tridium {
	...
	public:
		...
		JoltMeshCollider( const std::vector<Vector3> a_Vertices, const std::vector<uint32_t>& a_Indices, const AABBOld& a_BoundingBox );
		virtual ~JoltMeshCollider() = default;

		virtual bool IsValid() const override { return m_IsValid; }
		virtual void Generate( const std::vector<Vector3>& a_Vertices, const std::vector<uint32_t>& a_Indices, const AABBOld& a_BoundingBox ) override;
