...
#define _DRAW_INT(_id, _color) \
	...
	bool DrawProperty( const char* a_Name, Vector2& a_Value, EDrawPropertyFlags a_Flags )
	{
		...
		bool modified = false;
		{
			...
			ImGui::PushID( a_Name );
			{
				EditorStyle::Pallete& colors = Editor::GetStyle().Colors;

				ImGui::PushItemWidth( itemWidth );

				// x
				_DRAW_FLOAT( 0, colors.Red )

					ImGui::SameLine();

				// y
				_DRAW_FLOAT( 1, colors.Green )
	...
	bool DrawProperty( const char* a_Name, iVector2& a_Value, EDrawPropertyFlags a_Flags )
	{
		...
		bool modified = false;
		{
			...
			ImGui::PushID( a_Name );
			{
				...
				// x
				_DRAW_INT( 0, colors.Red )

				ImGui::SameLine();

				// y
				_DRAW_INT( 1, colors.Green )
