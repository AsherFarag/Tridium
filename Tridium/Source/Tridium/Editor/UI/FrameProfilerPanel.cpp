...
namespace Tridium {
	...
	void FrameProfilerPanel::OnDraw( StringView a_Name, bool& o_Open )
	{
		if ( ImGui::Begin( a_Name.data(), &o_Open ) )
		{
...

			{
				...
				ImGui::PushItemWidth( UI::CalcButtonSize( "Start Recording" ).X * 1.2f );

				if ( ImGui::InputInt( "Frames to Capture", &m_NumOfFramesToCapture ) )
				{
					if ( m_NumOfFramesToCapture < 0 )
						m_NumOfFramesToCapture = 0;
				}
				...
				if ( ImGui::ButtonEx( "Start Recording" ) )
				{
					...
					{
						Instrumentor::Get()->BeginSession( "Profiler", m_NumOfFramesToCapture );
					}
				}
	...
	void FrameProfilerPanel::DrawSession( ProfilerSession& a_Session )
	{
	...
	void FrameProfilerPanel::DrawThread( size_t a_ThreadID, const ProfilerSession::ProfiledThreadResult& a_ThreadData, TimelineData a_TimelineData )
	{
			...
			for ( const auto& frame : a_ThreadData.Frames )
			{
				for ( const auto& result : frame.CollectedResults )
				{
					...
					// Draw the event rectangle
					{
						...
						{
							ImGui::GetWindowDrawList()->AddRect( resultRect.Min, resultRect.Max, UI::GetTheme().Orange, 0.0f, 0, 3.0f);
						}
	...
	void FrameProfilerPanel::RetrieveRecordedSession()
	{
